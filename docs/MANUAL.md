...
| **Hermes** | One active provider at a time (a Hermes limitation). Choose with `HERMES_MODEL` (`provider/model`). Custom endpoints work via `OPENAI_API_KEY` + `OPENAI_BASE_URL` overrides. |
